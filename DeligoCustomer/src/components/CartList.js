    ...
    cartsArray.forEach((cart) => {
      ...
      if (products && products.length > 0) {
        const match = products.find((p) => {
          ...
          const vs = String(vid);
          return (
            String(v.vendorId) === vs ||
            String(v._id) === vs ||
            String(rv.vendorId) === vs ||
            String(rv._id) === vs
          );
        });
    ...
    <View style={{ padding: 16, position: "relative" }}>
      ...
      {cartsArray.map((cart, index) => {
        ...
        if (products && products.length > 0 && firstItem) {
          ...
          if (rawId)
            productContextMatch = products.find(
              (p) =>
                p.id === rawId ||
                p._id === rawId ||
        ...
        const pcVendor =
          productContextMatch?.vendor || productContextMatch?._raw?.vendor;
        const finalVendorName =
          pcVendor?.vendorName ||
          pcVendor?.name ||
          productContextMatch?.name ||
          cached?.name ||
          (cart.vendorName && !["Store", "Vendor"].includes(cart.vendorName)
            ? cart.vendorName
            : null) ||
          firstItem?._raw?.vendor?.vendorName ||
          t("store");
        const finalVendorImage =
          pcVendor?.storePhoto ||
          pcVendor?.logo ||
          pcVendor?.image ||
          productContextMatch?.image ||
          cached?.image ||
          cart.vendorImage ||
                    ...
                    {finalVendorImage ? (
                      ...
                      <View
                        style={[
                          styles.vendorImage,
                          {
                            backgroundColor: isDarkMode ? "#2A2A2A" : "#F5F0F3",
                            alignItems: "center",
