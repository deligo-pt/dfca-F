    ...
    try {
      ...
      if (token) {
        ...
        const authHeader = token.startsWith("Bearer ")
          ? token
          : `Bearer ${token}`;
        console.log("token value:", authHeader);

        config.headers.Authorization = authHeader;
        console.debug(
          "[api] request -> auth present, mask:",
    ...
    if (status === HTTP_STATUS.UNAUTHORIZED && !originalRequest._retry) {
      ...
      if (isRefreshing) {
        try {
          ...
          console.debug(
            "[api] retry queued request with token mask:",
            maskToken(authHeader),
          );
      ...
      refreshPromise = (async () => {
        try {
          ...
          console.debug(
            "[api] attempting refresh, refresh mask:",
          ...
          try {
            ...
            console.debug(
              "[api] body refresh failed or returned non-200",
          ...
          if (!newAccessToken) {
            try {
              ...
              const resp2 = await axios.post(refreshUrl, null, {
                withCredentials: true,
                headers: { "Content-Type": "application/json" },
              });
              newAccessToken =
                resp2?.data?.data?.accessToken ||
                resp2?.data?.accessToken ||
                resp2?.data?.token ||
              ...
              console.debug(
                "[api] cookie-only refresh failed",
                e?.response?.status || e.message,
              );
            }
          }
          ...
          try {
            await StorageService.setAccessToken(finalToken);
          } catch (e) {
            console.warn("[api] persist token failed", e);
          }
          ...
          if (newRefreshToken) {
            try {
              ...
              console.warn("[api] persist new refresh token failed", e);
            }
          }
          ...
          console.debug(
            "[api] retry original with new token mask:",
            maskToken(authHeader),
          );
          return customerApi(originalRequest);
        } catch (refreshError) {
