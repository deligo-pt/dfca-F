...
try {
  ...
  console.warn(
    "[LiveChatScreen] NetInfo native module not available:",
    error.message,
  );
}
...
const LiveChatScreen = () => {
    ...
    try {
      ...
      if (!token) {
        Alert.alert(
          ...
          t("pleaseLoginAgain") || "Please login again to access support chat.",
        );
        setIsLoading(false);
        return;
      }
      ...
      console.log(
        "[LiveChat] Parsing Room Data:",
        JSON.stringify(roomData, null, 2),
      );
      const room =
        roomData.room ||
        roomData._id ||
        (roomData.conversation ? roomData.conversation.room : null);
      console.log("[LiveChat] Resolved Room:", room);

      if (room) {
        setRoomId(room);
        const status =
          roomData.status || roomData.conversation?.status || "OPEN";
        const handledBy =
          roomData.handledBy || roomData.conversation?.handledBy;

        setChatStatus(status);
        if (handledBy) {
          const agentName =
            typeof handledBy === "string"
              ? "Agent"
              : handledBy.name?.firstName || handledBy.firstName || "Agent";
  ...
  const handleAttachment = async () => {
    ...
    try {
      ...
      Alert.alert(
        t("error") || "Error",
        t("filePickError") || "Could not pick file.",
      );
    }
  };
  ...
  const renderMessage = ({ item, index }) => {
    ...
    const nextSenderId =
      index < messages.length - 1 ? getMsgSenderId(messages[index + 1]) : null;
    const prevSenderId = index > 0 ? getMsgSenderId(messages[index - 1]) : null;

    const isFollowUp = index > 0 && currentSenderId === prevSenderId;
    const isLastInChain =
      index === messages.length - 1 || currentSenderId !== nextSenderId;

    const isUserMsg =
      isUserRole ||
      currentSenderId === myId ||
      currentSenderId === "user" ||
      item.role === "customer";
      ...
      if (dateStr === now.toDateString()) {
        ...
        if (dateStr === yesterday.toDateString()) {
          ...
          dateDividerLabel = currentMsgDate.toLocaleDateString([], {
            ...
            day: "numeric",
            year:
              currentMsgDate.getFullYear() !== now.getFullYear()
                ? "numeric"
                : undefined,
          });
        }
    ...
    return (
      <View key={item._id || item.id || index}>
        ...
        >
          {!isUserMsg && (
            <View
              style={[styles.avatarContainer, { opacity: isFollowUp ? 0 : 1 }]}
            >
              ...
              item.senderPhoto ? (
                <Image
                  ...
                  style={[
                    ...
                    { borderRadius: 14, opacity: isLastInChain ? 1 : 0 },
                  ]}
                />
              ) : (
                <LinearGradient
            ...
            {isUserMsg ? (
              ...
              <View
                style={[
                  ...
                  styles.agentBubble,
                  {
                    backgroundColor: colors.isDarkMode ? "#2C2C2C" : "#FFFFFF",
                  }, // Clean white for agents
          ...
          {isUserMsg && (
            ...
            >
              {profile?.profilePicture || profile?.profilePhoto ? (
                <Image
                  source={{
                    uri: profile?.profilePicture || profile?.profilePhoto,
                  }}
                  style={[
                    styles.avatarGradient,
                    { borderRadius: 14, opacity: isLastInChain ? 1 : 0 },
                ...
                <Image
                  source={{
                    uri: item.sender?.profilePicture || item.senderPhoto,
                  }}
                  style={[
                    styles.avatarGradient,
                    { borderRadius: 14, opacity: isLastInChain ? 1 : 0 },
  ...
  const getStatusText = () => {
    if (!isInternetReachable)
      return t("waitingForNetwork") || "Waiting for network...";
    ...
    >
          ...
          >
            <View style={{ flex: 1 }}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                ...
                >
                  <Ionicons
                    name={orderId ? "receipt" : "card"}
                    size={14}
                    color={colors.primary}
                  />
                </View>
                <Text
                  style={[styles.contextText, { color: colors.text.primary }]}
                >
                  {orderId ? (
                    <>
                      {t("order") || "Order"}{" "}
      ...
      <Animated.View
        style={[
          ...
          { height: offlineBannerHeight, backgroundColor: "#344955" },
        ]}
      >
        <Ionicons
      ...
      >
        ...
        >
          {isLoading ? (
            ...
            <View style={styles.centerContainer}>
              <View
                style={[styles.errorIconCircle, { backgroundColor: "#FFEBEE" }]}
              >
                <Ionicons
                  name="cloud-offline"
                  size={32}
              ...
              >
                {t("connFailedDesc") ||
                  "We couldn't connect to the support chat."}
              </Text>
              <TouchableOpacity
                style={[
                  styles.retryButton,
        ...
        {chatStatus === "CLOSED" ? (
          <View
            style={[styles.closedBanner, { backgroundColor: colors.surface }]}
          >
            <Ionicons
          ...
          >
            ...
            >
              <Ionicons
                ...
                size={28}
                color={
                  isInternetReachable ? colors.primary : colors.text.secondary
                }
              />
            </TouchableOpacity>
              ...
              <TextInput
                ...
                placeholder={
                  isInternetReachable
                    ...
                    : t("waitingNetwork") || "Waiting for network..."
                }
