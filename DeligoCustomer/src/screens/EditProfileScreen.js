...
const EditProfileScreen = ({ navigation, route }) => {
  ...
  const loadUserData = async () => {
    try {
      ...
      const addr =
        localUserData?.address || localUserData?.location || defaultAddress;
      setAddress(addr);
    ...
    try {
      ...
      if (error.response) {
        ...
        if (data?.err?.code === 11000) {
          ...
          throw new Error(
            t("duplicateDetailError") || "Account details already in use.",
          );
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedPhotoUrl =
        updateResponse?.customer?.profilePhoto ||
  ...
  const searchAddress = async () => {
    ...
    try {
      ...
      if (
        ...
        setStreetAddress(
          detailedPlace.formatted_address || detailedPlace.name || "",
        );
        ...
        if (detailedPlace.address_components) {
          cityComponent = detailedPlace.address_components.find(
            (component) =>
          ...
          countryComponent = detailedPlace.address_components.find(
    ...
    try {
      ...
      if (addresses && addresses.length > 0) {
        ...
        const addressParts = [
          ...
          (val, index, self) =>
            val && val.trim() !== "" && self.indexOf(val) === index,
        );
  ...
  const pickFromCamera = async () => {
    try {
      ...
      // First close the image picker modal
  ...
  const handleImageEditConfirm = (editedUri) => {
    console.log("Image edit confirmed, URI:", editedUri);

    if (!editedUri) {
      console.error("No edited URI provided");
      showModal(t("error"), t("failedToProcessImage"), null, true);
      return;
    }
        ...
        >
          ...
          >
            <View style={styles.imagePickerHeader}>
              <Text style={styles.imagePickerTitle}>{t("selectPhoto")}</Text>
            </View>
            <TouchableOpacity
