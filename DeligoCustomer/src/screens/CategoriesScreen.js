...
const CategoriesScreen = ({ navigation }) => {
  ...
  const cartVendorsCount =
    cartsArray && cartsArray.length ? cartsArray.length : 0; // number of vendor carts
    ...
    (async () => {
      ...
      try {
        ...
        if (fetchProductCategories) {
          ...
          console.log(
            "[CategoriesScreen] Fetched product categories:",
            pcats?.length,
          );
  ...
  const localNormalize = (p) => {
    ...
    const storePhoto =
      documents.storePhoto || vendorSource.storePhoto || vendorSource.logo;

    const vendorName =
      vendorSource.businessName ||
    ...
    let vendorType =
      vendorSource.businessType ||
      businessDetails.businessType ||
      vendorSource.vendorType ||
      "";
    ...
    const vendorRating =
      vendorSource.rating && typeof vendorSource.rating === "number"
        ? vendorSource.rating
        : 0;

    const price =
      p.pricing && typeof p.pricing.price !== "undefined"
        ? p.pricing.price
        : raw.price || 0;
    ...
    return {
      ...
      offer:
        p.pricing && p.pricing.discount
          ? `${p.pricing.discount}% ${t("off")}`
          : null,
      vendor: {
  ...
  if (user) {
    ...
    if (typeof user.name === "object" && user.name !== null) {
      ...
      if (!userName && user.name.fullName)
        userName = user.name.fullName.split(" ")[0];
  ...
  const cuisinesForSelectedVendor = React.useMemo(() => {
    ...
    const selected = selectedVendorType
      ? String(selectedVendorType).toLowerCase().trim()
      : null;
    ...
    for (const p of arr) {
      ...
      const catRaw =
        p._raw?.category ??
        p.category ??
        (Array.isArray(p.tags) ? p.tags[0] : null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const storedSelVendor =
          await StorageService.getItem("selectedVendorType");
        const storedSelCuisine =
          await StorageService.getItem("selectedCuisine");
        const storedSelBusinessId = await StorageService.getItem(
        ...
        const normalizeList = (input) => {
          ...
          if (Array.isArray(input)) {
            if (input.every((i) => typeof i === "string"))
              return input.map((s) => ({ id: s, name: s }));
            return input.map((i) =>
              typeof i === "string"
                ? { id: i, name: i }
                : {
        ...
        if (storedSelBusinessId)
          setSelectedBusinessCategoryId(storedSelBusinessId);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (
          (!staticVendorTypes || staticVendorTypes.length === 0) &&
          vendorTypesFromProducts.length > 0
        ) {
          setStaticVendorTypes(vendorTypesFromProducts);
          // store normalized array of objects
          await StorageService.setItem(
            "vendorTypes",
            vendorTypesFromProducts.map((v) => ({ id: v.id, name: v.name })),
          );
        }
        const baseCuisines = selectedVendorType
          ? cuisinesForSelectedVendor
          : cuisinesFromProducts;
        if (
          (!staticCuisines || staticCuisines.length === 0) &&
          baseCuisines.length > 0
        ) {
  ...
  const vendorTypes =
    Array.isArray(staticVendorTypes) && staticVendorTypes.length
      ? staticVendorTypes
      : Array.isArray(vendorTypesFromProducts)
        ? vendorTypesFromProducts
        : [];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        if (typeof stored === "number" && !isNaN(stored) && stored > 0)
          setCacheTtlMs(stored);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const filtered = arr.filter((p) => {
      ...
      const matchVendor =
        !selectedVendorType || (vt && vt.includes(selectedVendorType));

      if (selectedVendorType && !matchVendor && logCount < 3) {
        console.log(
          `[FilterDebug] Fail Vendor Match. Prod: ${p.name}, VT: '${vt}', Selected: '${selectedVendorType}'`,
        );
        logCount++;
      }
      ...
      if (!matchCuisine) {
        const checkCuisine = (c) => {
          ...
          if (typeof c === "object") {
            ...
            return (
              name === targetCuisine ||
              name.includes(targetCuisine) ||
              slug === targetCuisine ||
              slug.includes(targetCuisine) ||
              id === targetCuisine
            );
          }
          return false;
  ...
  const toVendorCard = React.useCallback((p) => {
    ...
    const storePhoto =
      documents.storePhoto ||
      vendorMerged.storePhoto ||
      vendorMerged.logo ||
      null;
    ...
    return {
      ...
      vendor: {
        ...
        vendorType:
          vendorMerged.businessType ||
          businessDetails.businessType ||
          vendorMerged.vendorType,
  ...
  const filteredVendors = React.useMemo(() => {
    ...
    const businessSlug = selectedVendorType
      ? String(selectedVendorType).toLowerCase().trim()
      : null;
    const cuisineSlug = selectedCuisine
      ? String(selectedCuisine).toLowerCase().trim()
      : null;
        ...
        if (cuisineSlug) {
          ...
          const checkCuisine = (c) => {
            ...
            if (typeof c === "object") {
              ...
              return (
                name === cuisineSlug ||
                name.includes(cuisineSlug) ||
                slug === cuisineSlug ||
                slug.includes(cuisineSlug)
              );
            }
            return false;
          };
  ...
  const displayCategories =
    apiCategories.length > 0 ? apiCategories : vendorTypesFromProducts;

  console.log(
    "[CategoriesScreen] Render. Selected:",
    selectedVendorType,
    "SelectedBusinessId:",
    selectedBusinessCategoryId,
    "ApiCats:",
  ...
  const handleVendorTypePress = React.useCallback(
    (vendor) => {
      ...
      const isSame =
        (selectedVendorType && selectedVendorType.toLowerCase()) === vendorSlug;
  ...
  const handleCuisinePress = React.useCallback(
    (cuisine) => {
  ...
  const handleLocationPress = () => {
    console.log(
      "📍 LOCATION BUTTON PRESSED - Navigating to LocationAddress screen",
    );
...
const styles = (colors) =>
  StyleSheet.create({
    ...
    errorContent: {
      alignItems: "center",
      marginTop: -60, // Lift it up slightly
    },
    errorIconBadge: {
      width: 100,
      height: 100,
