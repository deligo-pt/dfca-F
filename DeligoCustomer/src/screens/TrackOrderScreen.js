...
const TrackOrderScreen = ({ route, navigation }) => {
  ...
  const [initialOrder, setInitialOrder] = useState(() => {
    ...
    if (notificationData?.data) {
      return {
        ...
        orderStatus:
          notificationData.data.status || notificationData.data.orderStatus,
  ...
  const initialStatus = order?.orderStatus
    ? mapOrderStatusToStage(order.orderStatus)
    : "pending";
    ...
    const fetchGoogleEta = async (origin, destination) => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            `[TrackOrder] Google ETA: ${minutes} mins (${duration.text})`,
          );
          ...
          console.warn(
            "[TrackOrder] Google Distance Matrix API error:",
            data.status,
            data.error_message,
          );
          return false;
  ...
  useEffect(() => {
    const checkLocalStatus = async () => {
      if (orderId) {
        const isRatedLocally = await StorageService.getItem(
          `rating_completed_${orderId}`,
        );
        if (isRatedLocally) {
    ...
    const fetchOrderDetails = async () => {
      ...
      if (!paramOrder && paramOrderId) {
        try {
          ...
          console.log(
            "[TrackOrder] Fetching order details for ID:",
            paramOrderId,
          );

          // Use the GET_BY_ID endpoint
          const url = API_ENDPOINTS.ORDERS.GET_BY_ID.replace(
            ":id",
            paramOrderId,
          );
          ...
          if (isActive) {
            if (response && response.success && response.data) {
              ...
              setCurrentStatus(
                mapOrderStatusToStage(response.data.orderStatus),
              );
            } else {
              console.warn(
                "[TrackOrder] Failed to fetch order or no data returned",
              );
          ...
          if (isActive) {
            Alert.alert(
              t("error"),
              t("failedToLoadOrder") || "Failed to load order details",
              [{ text: "OK", onPress: () => navigation.goBack() }],
            );
          }
        } finally {
  ...
  useEffect(() => {
    ...
    const shouldPoll =
      order?._id &&
      !isDriverLive &&
      ["PICKED_UP", "ON_THE_WAY", "NEARBY", "ASSIGNED"].includes(
        order?.orderStatus,
      );
    ...
    const pollInterval = setInterval(async () => {
      ...
      try {
        ...
        console.log(
          `[TrackOrder] Smart Polling: Fetching snapshot for ID: ${idToUse}`,
        );
        ...
        if (response && response.success && response.data) {
          ...
          if (newPartner?.currentSessionLocation?.coordinates) {
            ...
            console.log(
              "[TrackOrder] Smart Polling: Got new snapshot coords:",
              newCoords,
            );
  ...
  const normalizeOrderData = (data) => {
    ...
    const normalizedItems = (data.items || []).map((item) => {
      ...
      return {
        ...
        quantity: quantity,
        price:
          item.productPricing?.unitPrice ??
          (item.price || subtotal / quantity || 0),
        subtotal: subtotal,
      };
    });
    ...
    if (data.createdAt) {
      try {
        ...
        orderTime = date.toLocaleTimeString("en-US", {
          hour: "2-digit",
          minute: "2-digit",
        });
      } catch (e) {
    ...
    if (
      data.pickupAddress &&
      typeof data.pickupAddress.latitude === "number" &&
      typeof data.pickupAddress.longitude === "number"
    ) {
      restaurantCoords = {
      ...
      console.log(
        "[TrackOrder] Using pickupAddress for restaurant location:",
        restaurantCoords,
      );
    }
    ...
    if (!restaurantCoords && vendorObj) {
      if (
        typeof vendorObj.latitude === "number" &&
        typeof vendorObj.longitude === "number"
      ) {
        restaurantCoords = {
        ...
        vendorObj.businessLocation &&
        vendorObj.businessLocation.coordinates
      ) {
        restaurantCoords = {
        ...
        vendorObj.businessLocation &&
        typeof vendorObj.businessLocation.latitude === "number"
      ) {
        restaurantCoords = {
    ...
    if (!restaurantCoords && data.items && data.items.length > 0) {
      ...
      if (product && typeof product === "object") {
        ...
        if (productVendor && typeof productVendor === "object") {
          if (
            productVendor.businessLocation &&
            typeof productVendor.businessLocation.latitude === "number"
          ) {
            restaurantCoords = {
            ...
            Array.isArray(productVendor.location.coordinates)
          ) {
            restaurantCoords = {
          ...
          if (!restaurantName || restaurantName === "Restaurant") {
            restaurantName =
              productVendor.businessDetails?.businessName ||
    ...
    if (data.deliveryPartnerId && typeof data.deliveryPartnerId === "object") {
      ...
      driverPhone =
        partner.phone || partner.phoneNumber || partner.contactNumber || "";
    ...
    return {
      ...
      restaurantAddress: data.pickupAddress
      ...
      totalItems:
        data.totalItems ??
        normalizedItems.reduce((acc, item) => acc + (item.quantity || 1), 0),
      subtotal: Number(
        data.orderCalculation?.taxableAmount ??
          data.orderCalculation?.totalOriginalPrice ??
          calculatedSubtotal ??
          0,
      ),
      taxAmount: Number(data.orderCalculation?.totalTaxAmount ?? 0),
      deliveryFee: Number(
        data.delivery?.totalDeliveryCharge ?? data.deliveryCharge ?? 0,
      ),
      serviceFee: data.serviceFee || 0,
      discount: data.discount || 0,
      totalAmount: Number(
        data.payoutSummary?.grandTotal ?? data.totalAmount ?? 0,
      ),
    ...
    () => [
      {
        ...
        subtitle:
          t("waitingForRestaurant") || "Waiting for restaurant response",
        icon: "time",
      ...
      },
      {
        ...
        subtitle:
          order?.orderStatus === "AWAITING_PARTNER" ||
          order?.orderStatus === "DISPATCHING" ||
          order?.orderStatus === "REASSIGNMENT_NEEDED"
      ...
      },
      {
        ...
        subtitle:
          order?.orderStatus === "ASSIGNED"
            ? t("driverAssignedPreparing") ||
              "Driver assigned • Preparing your order"
  ...
  const handleCallDriver = () => {
    const phoneNumber = orderData.driverPhone
      ? orderData.driverPhone.replace(/\D/g, "")
      : "";
  ...
  const handleCallRestaurant = () => {
    const phoneNumber = orderData.restaurantPhone
      ? orderData.restaurantPhone.replace(/\D/g, "")
      : "";
  ...
  const confirmCall = () => {
    ...
    const phoneUrl =
      Platform.OS === "ios"
        ? `telprompt:${callTarget.cleanPhone}`
        : `tel:${callTarget.cleanPhone}`;
  ...
  const showQuickMessages = () => {
    Alert.alert(t("quickMessages"), t("selectMessage"), [
      {
        text: t("whereAreYou"),
    ...
    (async () => {
      try {
        ...
        if (order?.deliveryAddress) {
          ...
          if (
            typeof da.latitude === "number" &&
            typeof da.longitude === "number"
          ) {
            orderDeliveryCoords = {
            ...
            orderDeliveryCoords = {
              latitude: da.location.coordinates[1],
              longitude: da.location.coordinates[0],
            };
          } else if (da.coordinates && Array.isArray(da.coordinates)) {
            orderDeliveryCoords = {
              latitude: da.coordinates[1],
              longitude: da.coordinates[0],
            };
          }
        }
        ...
        if (orderDeliveryCoords) {
          console.log(
            "[TrackOrder] Using real delivery coordinates from order:",
            orderDeliveryCoords,
          );
          ...
          if (
            ...
            if (geocodingCache.current[addressKey]) {
              ...
              console.log(
                "[TrackOrder] Using cached delivery coordinates:",
                cachedCoords,
              );
              if (active) {
              ...
              try {
                ...
                if (geocoded && geocoded.length > 0) {
                  ...
                  console.log(
                    "[TrackOrder] Geocoded delivery address:",
                    geoCoords,
                  );
        ...
        if (realRestCoords) {
          console.log(
            "[TrackOrder] Using real restaurant coordinates:",
            realRestCoords,
          );
          if (active) setRestaurantLocation(realRestCoords);
        } else {
          console.warn(
            "[TrackOrder] No restaurant coordinates available - will attempt to fetch from vendor API",
          );
        ...
        if (partner && typeof partner === "object") {
          ...
          if (partner.location && Array.isArray(partner.location.coordinates)) {
            ...
            if (Array.isArray(cl.coordinates)) {
              partnerCoords = {
                latitude: cl.coordinates[1],
                longitude: cl.coordinates[0],
              };
            ...
            if (Array.isArray(csl.coordinates)) {
              partnerCoords = {
                latitude: csl.coordinates[1],
                longitude: csl.coordinates[0],
              };
            } else if (typeof csl.latitude === "number") {
              partnerCoords = {
                latitude: csl.latitude,
                longitude: csl.longitude,
              };
            }
          }
          ...
          if (partnerCoords) {
            console.log(
              "[TrackOrder] Using real driver coordinates from order:",
              partnerCoords,
            );
            if (active) {
            ...
            console.log(
              "[TrackOrder] Driver assigned but no location data - waiting for socket updates",
            );
          }
        } else {
  ...
  useEffect(() => {
    const fetchVendorDetails = async () => {
      ...
      if (orderData.restaurantCoordinates) {
        ...
        if (
          ...
          restaurantLocation.latitude !==
            orderData.restaurantCoordinates.latitude ||
          restaurantLocation.longitude !==
            orderData.restaurantCoordinates.longitude
        ) {
          console.log(
            "[TrackOrder] Vendor: Updating coords from orderData:",
      ...
      if (restaurantLocation) {
        console.log(
          "[TrackOrder] Vendor: Already have restaurant location, skipping fetch",
        );
        return;
      }
      ...
      if (order?.items && order.items.length > 0) {
        for (const item of order.items) {
          ...
          if (vendor && typeof vendor === "object") {
            console.log(
              "[TrackOrder] Vendor: Found in order items:",
              vendor._id,
            );
            const coords = extractVendorCoords(vendor);
            if (coords) {
              console.log(
                "[TrackOrder] Vendor: Extracted coords from order items:",
                coords,
              );
      ...
      console.log(
        "[TrackOrder] Vendor: order.vendorId type:",
        typeof order?.vendorId,
        "value:",
        JSON.stringify(order?.vendorId, null, 2)?.substring(0, 800),
      );
      ...
      if (order?.vendorId && typeof order.vendorId === "object") {
        ...
        if (coords) {
          console.log(
            "[TrackOrder] Vendor: Extracted coords from order.vendorId:",
            coords,
          );
          setRestaurantLocation(coords);
          return;
          ...
          console.log(
            "[TrackOrder] Vendor: vendorId object has no coords, businessLocation:",
      ...
      console.log(
        "[TrackOrder] Vendor: First item structure:",
      ...
      console.log(
        "[TrackOrder] Vendor: Product identifier:",
        productIdentifier,
        "(SKU:",
        productSku,
        ", ObjectId:",
      ...
      if (productIdentifier) {
        try {
          console.log(
            "[TrackOrder] Vendor: Fetching product:",
            productIdentifier,
          );
          ...
          console.log(
            "[TrackOrder] Vendor: API Response keys:",
          ...
          if (product) {
            console.log(
              "[TrackOrder] Vendor: Product found:",
          ...
          if (vendor) {
            console.log(
              "[TrackOrder] Vendor: Extracted vendor object:",
              vendor._id,
              vendor.businessDetails?.businessName,
            );
            const coords = extractVendorCoords(vendor);
            if (coords) {
              ...
              console.warn(
                "[TrackOrder] Vendor: Found vendor but no valid coords",
              );
            }
          } else {
            console.warn(
              "[TrackOrder] Vendor: No vendor found in product response",
            );
          }
        } catch (err) {
          console.warn(
            "[TrackOrder] Vendor: Product fetch failed:",
            err.message,
          );
        }
      } else {
        console.log(
          "[TrackOrder] Vendor: No product identifier available in order items",
        );
      }
    };
    ...
    const extractVendorCoords = (vendor) => {
      ...
      if (
        ...
        typeof vendor.businessLocation.latitude === "number"
      ) {
        return {
  ...
  const safeFitToCoordinates = (
    ...
    fittingTimeoutRef.current = setTimeout(() => {
      if (ref.current) {
        try {
          console.log(
            `[TrackOrder] Power Fitting ${ref === inlineMapRef ? "inline" : "fullscreen"} map to ${coords.length} points`,
          );
  ...
  useEffect(() => {
    ...
    if (points.length > 1) {
      ...
      if (isMapFullscreen) {
        setTimeout(() => {
          safeFitToCoordinates(fullscreenMapRef, fullscreenMapLayout, points, {
            top: 100,
            right: 50,
            bottom: 100,
            left: 50,
  ...
  useEffect(() => {
    ...
    console.log(
      "[TrackOrder] [Socket] Setting up live tracking for order:",
      order._id,
    );
    console.log("[TrackOrder] [Socket] Socket connected:", socket.id);

    // Join the specific order room
    console.log("[TrackOrder] [Socket] Joining room: join-order-tracking", {
      orderId: order._id,
    });
    ...
    const handleLocationUpdate = (data) => {
      console.log(
        "[TrackOrder] [Socket] 📍 Received live driver location data:",
        JSON.stringify(data),
      );
    ...
    return () => {
      console.log(
        "[TrackOrder] [Socket] Cleaning up socket listeners for order:",
        order._id,
      );
  ...
  useEffect(() => {
    ...
    const statusIndex = orderStages.findIndex(
            ...
            <View style={{ width: "100%", gap: 12, marginTop: 20 }}>
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  Alert.alert(t("chat"), t("chatFeatureText"), [
                    { text: "OK" },
                  ]);
                }}
              >
      ...
      return (
        <View style={styles.mapContainer}>
          <View style={[styles.map, styles.mapLoading]}>
            ...
            />
            <Text
              style={[styles.mapLoadingText, { color: colors.text.secondary }]}
            >
        ...
        >
          {orderStages.map((stage, index) => {
            ...
            return (
              <View
                key={stage.id}
                style={{ flexDirection: "row", minHeight: 60 }}
              >
                {/* Left Timeline Column */}
                <View
                  style={{ alignItems: "center", width: 40, marginRight: 16 }}
                >
                  {/* Top Connector Line */}
                  {index > 0 && (
                    <View
                    ...
                    >
                      {current ? (
                        ...
                        <Ionicons
                          ...
                          size={16}
                          color={
                            completed ? colors.text.white : colors.text.light
                          }
                        />
                      ) : (
                        <MaterialIcons
                ...
                >
                  <Text
                    style={{
                      fontSize: 16,
                      fontFamily: current ? "Poppins-Bold" : "Poppins-SemiBold",
                      color: current ? colors.primary : colors.text.primary,
                      letterSpacing: -0.3,
                    }}
                  >
                    {stage.title}
                  ...
                  >
                    ...
                    <Text
                      ...
                      numberOfLines={2}
                    >
                      {stage.subtitle && stage.subtitle !== "findingDriver"
                        ? stage.subtitle
                        : t("findingDriver") !== "findingDriver"
                          ? t("findingDriver")
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
            ...
            >
              {orderData.driverName === t("awaitingDriver")
                ? t("assigningDriver") || "Assigning Driver..."
                : orderData.driverName}
            </Text>
            {isDriverLive ? (
              <View
          ...
          <View style={{ flex: 1 }}>
            ...
            {orderData.discount > 0 && (
              ...
              >
                ...
                >
                  -€
                  {orderData.discount ? orderData.discount.toFixed(2) : "0.00"}
                </Text>
              </View>
            )}
      ...
      <SafeAreaView style={styles.container} edges={["top"]}>
        ...
        >
          ...
          <View style={{ paddingHorizontal: spacing.md }}>
            ...
            <SkeletonBlock
              style={{ width: "100%", height: 2, marginBottom: spacing.lg }}
            />
            ...
            <View style={{ gap: 24, marginBottom: spacing.xl }}>
              {[1, 2, 3].map((i) => (
                ...
                >
                  ...
                  <View style={{ gap: 6 }}>
                    <SkeletonBlock
                      style={{ width: 160, height: 18, borderRadius: 4 }}
                    />
                    <SkeletonBlock
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      >
        ...
        {(currentStatus === "picked_up" ||
          ...
          (currentStatus === "preparing" && orderData.driverName)) &&
          renderDriverInfo()}
            ...
            >
              <TouchableOpacity
                style={[
                  ...
                  { backgroundColor: colors.secondary, marginTop: 0 },
                ]} // Using secondary color or gold for rating
      ...
      >
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <Text style={[styles.modalPhone, { color: colors.text.secondary }]}>
              {callTarget.phone}
            </Text>
      ...
      <OrderRatingModal
        ...
        driverName={
          orderData.driverName !== t("awaitingDriver")
            ? orderData.driverName
            : currentStatus === "delivered"
              ? t("deliveryRider") || "Delivery Partner"
              : null
        }
        onRatingSuccess={() => {
